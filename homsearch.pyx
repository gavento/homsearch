...
cdef extern from "homsearch-lib.h":
    ...
    cdef cppclass homsearch32:
        ...
        bool res_store
        bool retract_mode
        homsearch32(vector[vector[int]] &G_, vector[vector[int]] &H_, long long int res_limit_, bool res_store_, int max_depth_)
        void search(search_state32 &s, int depth)

cdef class Homsearch:
    pass

cdef class Homsearch32(Homsearch):
    cdef homsearch32 *search32

    def __init__(self, G_adj, H_adj, res_limit, res_store, max_depth=-1):
#        max_size = max(len(G_adj), len(H_adj))
#        assert (max_size <= 32)
#        self.search32 = new homsearch32(G_adj, H_adj, res_limit, res_store, max_depth)
        pass
...
cdef class Homsearch512(Homsearch):
    pass

def Homsearch_create(G_adj, H_adj, res_limit, res_store, max_depth=-1):
    max_size = max(len(G_adj), len(H_adj))
    if max_size <= 32:
        return Homsearch32(G_adj, H_adj, res_limit, res_store, max_depth)
    if max_size <= 512:
        return Homsearch512(G_adj, H_adj, res_limit, res_store, max_depth)
    raise Exception("Graph too big for homcore lib (max size 512)")

Homsearch_create([],[],0,0,0)
